...
struct Vertex{
	...
	static VkVertexInputBindingDescription getBindingDescription(){
		...
		bindingDescription.binding = 0; //first binding is at the 0th location in array
...
private:
	...
	void createVertexBuffer(){
		...
		if(vkCreateBuffer(device, &bufferInfo, nullptr, &vertexBuffer) != VK_SUCCESS) throw std::runtime_error("Failed to create vertex buffer.\n");
		...
		if(vkAllocateMemory(device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) throw std::runtime_error("Failed to allocate vertex buffer memory.\n");
